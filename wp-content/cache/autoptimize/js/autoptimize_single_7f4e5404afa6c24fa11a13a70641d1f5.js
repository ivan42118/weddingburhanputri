jQuery(document).ready(function ($) {
  ...
  jQuery(
    '.wdp-container-form [name="comment_parent"], .wdp-container-form [name="comment_post_ID"]'
  ).each(function (index, input) {
    $(input).removeAttr("id");
  });
  if (
    typeof jQuery.fn.textareaCount == "function" &&
    WDP.textCounter == "true"
  ) {
  ...
  if (typeof jQuery.fn.placeholder == "function") {
    $(
      ".wdp-wrap-form input, .wdp-wrap-form textarea, #wdp-modal input, #wdp-modal textarea"
    ).placeholder();
  }
    ...
    if ($content.length < 2) {
      ...
      if (form_ok === true) {
        ...
        if (form.find('input[name="submit"]').hasClass("wdp-edit-action")) {
          ...
          form.find('input[name="submit"]').hasClass("wdp-reply-action")
        ) {
  ...
  function getComments_WDP(
    ...
    if (num_comments > 0) {
      jQuery.ajax({
        ...
        beforeSend: function () {
          status
            .addClass("wdp-loading")
            .html('<span class="wdpo-loading"></span>')
            .show();
        },
  ...
  function highlightPopularComments_WDP(post_id, $container_comments) {
    ...
    if (
      order == "likes" &&
      $container_comments.hasClass("wdp-multiple-comments wdp-has-likes")
    ) {
      var top_likes = $container_comments
        .find(">.wdp-item-comment")
        .eq(0)
        .data("likes");
      var temp = false;
      $container_comments
  ...
  function insertComment_WDP(post_id, num_comments) {
    ...
    $.ajax({
      ...
      success: function (data, textStatus) {
        ...
        if (data != "error") {
          status.html('<p class="wdp-ajax-success">Thank You!</p>');
          ...
          status.html(
            '<p class="wdp-ajax-error">Error processing your form</p>'
          );
        }
      ...
      error: function (XMLHttpRequest, textStatus, errorThrown) {
        status
          .removeClass("wdp-loading")
          .html('<p class="wdp-ajax-error" >' + WDP.duplicateComment + "</p>");
      },
      complete: function (jqXHR, textStatus) {
        setTimeout(function () {
  ...
  function insertCommentReply_WDP(post_id, comment_id, num_comments) {
    ...
    $.ajax({
      ...
      success: function (data, textStatus) {
        ...
        if (data != "error") {
          status.html(
            '<p class="wdp-ajax-success">' + WDP.thanksReplyComment + "</p>"
          );
          if (link_show_comments.find("span").length) {
          ...
          status.html(
            '<p class="wdp-ajax-error">Error in processing your form.</p>'
          );
        }
      },
      error: function (XMLHttpRequest, textStatus, errorThrown) {
        status.html(
          '<p class="wdp-ajax-error" >' + WDP.duplicateComment + "</p>"
        );
      },
      complete: function (jqXHR, textStatus) {
        setTimeout(function () {
  ...
  function toggleMoreComments($container_comments) {
    ...
    liComments.each(function (index, element) {
      ...
      if (ulChildren.length && ulChildren.find("li").length > 3) {
        ...
        ulChildren.append(
          '<a href="#" class="wdp-load-more-comments">' +
            WDP_WP.textLoadMore +
            "</a>"
        );
      }
  ...
  $(document).delegate(".wdp-media-btns a", "click", function (e) {
    ...
    $modalHtml =
      '<div id="wdp-modal-wrap"><span id="wdp-modal-close"></span><div id="wdp-modal-header"><h3 id="wdp-modal-title">Título</h3></div><div id="wdp-modal-content"><p>Hola</p></div><div id="wdp-modal-footer"><a id="wdp-modal-ok-' +
      post_id +
      '" class="wdp-modal-ok wdp-modal-btn" href="#">' +
      WDP.accept +
      '</a><a class="wdp-modal-cancel wdp-modal-btn" href="#">' +
      WDP.cancel +
      "</a></div></div>";
    ...
    switch ($action) {
      case "url":
        ...
        $("#wdp-modal-content").html(
          '<input type="text" id="wdp-modal-url-link" class="wdp-modal-input" placeholder="' +
            WDP_WP.textUrlLink +
            '"/><input type="text" id="wdp-modal-text-link" class="wdp-modal-input" placeholder="' +
            WDP_WP.textToDisplay +
            '"/>'
        );
        break;
      case "image":
        ...
        $("#wdp-modal-content").html(
          '<input type="text" id="wdp-modal-url-image" class="wdp-modal-input" placeholder="' +
            WDP_WP.textUrlImage +
            '"/><div id="wdp-modal-preview"></div>'
        );
        break;
      case "video":
        ...
        $("#wdp-modal-content").html(
          '<input type="text" id="wdp-modal-url-video" class="wdp-modal-input" placeholder="' +
            WDP_WP.textUrlVideo +
            '"/><div id="wdp-modal-preview"></div>'
        );
        $("#wdp-modal-footer").prepend(
          '<a id="wdp-modal-verifique-video" class="wdp-modal-verifique wdp-modal-btn" href="#">' +
            WDP.checkVideo +
            "</a>"
        );
        break;
  ...
  $(document).delegate(
    "#wdp-modal input, #wdp-modal textarea",
    "focus",
    function (e) {
      $(this).removeClass("wdp-error");
    }
  ...
  $(document).delegate("#wdp-modal-url-image", "change", function (e) {
    setTimeout(function () {
      $("#wdp-modal-preview").html(
        '<img src="' + $("#wdp-modal-url-image").val() + '" />'
      );
    }, 200);
  });
  function processVideo_WDP(post_id) {
    ...
    if (!$("#wdp-modal-preview").find("iframe").length) {
      $ok = false;
      $("#wdp-modal-preview").html(
        '<p class="wdp-modal-error">Please check the video url</p>'
      );
    }
    if ($ok) {
      var $video =
        "<p>" +
        $("#wdp-modal-preview").find('input[type="hidden"]').val() +
        "</p>";
  ...
  $(document).delegate("#wdp-modal-verifique-video", "click", function (e) {
    ...
    if ($urlVideoVal.length < 1) {
      ...
      $(".wdp-modal-video")
        .find("a.wdp-modal-verifique")
        .attr("id", "wdp-modal-verifique-video");
      return false;
    }
    ...
    $.ajax({
      ...
      beforeSend: function () {
        $("#wdp-modal-preview").html(
          '<div class="wdp-loading wdp-loading-2"></div>'
        );
      },
      success: function (data) {
        if (data != "error") {
          ...
          $("#wdp-modal-preview").html(
            '<p class="wdp-modal-error">Invalid video url</p>'
          );
        }
      },
      error: function (xhr) {
        $("#wdp-modal-preview").html(
          '<p class="wdp-modal-error">Failed to process, try again</p>'
        );
      },
      complete: function (jqXHR, textStatus) {
        $(".wdp-modal-video")
  ...
  $(document).delegate(
    "#wdp-modal-close, .wdp-modal-cancel",
    "click",
    function (e) {
      e.preventDefault();
      closeModal_WDP();
  ...
  function restoreIframeHeight(wrapper) {
    var widthWrapper = WDP.widthWrap
      ? parseInt(WDP.widthWrap, 10)
      : wrapper.outerWidth();
  }
  ...
  function commentRating_WDP(comment_id, $method) {
    var $ratingCount = $("#wdp-comment-" + comment_id).find(
      ".wdp-rating-count"
    );
    ...
    jQuery.ajax({
      ...
      success: function (result) {
        ...
        if (data.success === true) {
          $ratingCount
            .html(data.likes)
            .attr("title", data.likes + " " + WDP_WP.textLikes);
          if (data.likes < 0) {
            $ratingCount
              .removeClass()
              .addClass("wdp-rating-count wdp-rating-negative");
          } else if (data.likes > 0) {
            $ratingCount
              .removeClass()
              .addClass("wdp-rating-count wdp-rating-positive");
          } else {
            $ratingCount
              .removeClass()
              .addClass("wdp-rating-count wdp-rating-neutral");
          }
        } else {
